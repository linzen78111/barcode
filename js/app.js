...
function updateLocalDataList() {
    console.log('更新本地暫存列表 - 已設為隱藏');
    
    // 不顯示本地暫存頁面和元素
    let localDataPage = document.getElementById('localDataPage');
    if (localDataPage) {
        localDataPage.classList.add('hidden');
    }
    
    // 維持功能性但不顯示
    localDataCount = document.getElementById('localDataCount');
    if (localDataCount) {
...
function initializeDropZone() {
    console.log('初始化檔案上傳區域');
    // 如果需要實現拖放功能，可以在這裡添加相關代碼
}

// 請求全螢幕模式 (停用)
function requestFullscreen() {
    console.log('全螢幕功能已停用');
    // 不執行任何操作
}

document.addEventListener("DOMContentLoaded", () => {
    console.log("DOM 已完全載入，開始初始化...");
    
    // 載入本地暫存資料
    console.log("載入本地暫存資料...");
    loadLocalBarcodes();
    
    // 初始化各項功能
    if (typeof initializeDropZone === 'function') {
        initializeDropZone();
    }
    
    if (typeof checkAndShowUploadButton === 'function') {
    ...
    // 添加暫存管理按鈕
    const uploadButton = document.getElementById("uploadButton");
    console.log("嘗試找到上傳按鈕：", uploadButton);
    
    if (uploadButton && uploadButton.parentNode) {
        console.log("找到上傳按鈕，準備創建管理暫存按鈕");
        console.log("上傳按鈕的父元素：", uploadButton.parentNode);
        ...
        manageButton.addEventListener("click", function(e) {
            console.log("管理暫存按鈕被點擊了！");
            console.log("點擊事件對象：", e);
            e.stopPropagation(); // 防止事件冒泡
            e.preventDefault(); // 防止默認行為
            showLocalDataManager();
        });
        
        // 插入到上傳按鈕前面
        uploadButton.parentNode.insertBefore(manageButton, uploadButton);
        console.log("管理暫存按鈕已創建並插入到 DOM，可見性：", manageButton.offsetParent !== null);
        ...
        manageButton.style.fontSize = "16px";
    } else {
        console.error("無法找到上傳按鈕，無法創建管理暫存按鈕");
    }
    
    // 點擊對話框背景關閉
    document.addEventListener("click", (e) => {
        if (e.target && e.target.classList && e.target.classList.contains("modal-overlay")) {
            // 處理其他模態對話框的關閉，但跳過本地暫存頁面
            const uploadModal = document.getElementById("uploadModal");
            if (uploadModal) {
                uploadModal.classList.add("hidden");
            }
    ...
    if (uploadBtn) {
        uploadBtn.addEventListener("click", async () => {
            ...
            // let localDataPage = document.getElementById("localDataPage");
            // if (!localDataPage) {
            //     // 如果頁面不存在，先更新本地列表會自動創建頁面
            //     updateLocalDataList();
            //     localDataPage = document.getElementById("localDataPage");
            // }
            
            // if (localDataPage) {
            //     localDataPage.classList.remove("hidden");
            // }
        });
    } else {
        console.error("找不到上傳按鈕");
    }
    
    // 確認本地數據相關元素是否存在
    const localDataList = document.getElementById('localDataList');
    const localDataCount = document.getElementById('localDataCount');
    console.log('本地數據相關元素檢查:', {
